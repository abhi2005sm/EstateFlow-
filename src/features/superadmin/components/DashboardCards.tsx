...
export default function DashboardCards() {
  ...
  const cards = [
    { title: 'Total Buildings', value: buildings, icon: Building2, color: 'text-blue-600 dark:text-white', bg: 'bg-blue-100' },
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {cards.map((card, index) => (
          <div key={index} className="bg-white dark:bg-[#18181b] p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center space-x-4">
            <div className={`p-3 rounded-full ${card.bg}`}>
              <card.icon className={`w-6 h-6 ${card.color}`} />
            </div>
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{card.title}</p>
              <h3 className="text-[#121110] dark:text-whitexl font-bold text-gray-800 dark:text-white">{card.value}</h3>
            </div>
          </div>
        ))}
      </div>
