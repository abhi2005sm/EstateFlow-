...
import { LayoutDashboard, Building2, LogOut, Users, Receipt, Settings, PieChart, ChevronDown, PlusCircle, Wrench, Shield, Menu, X } from 'lucide-react';
...
export default function AdminSidebar() {
  ...
  return (
    <>
      {/* Sidebar (Desktop Only now since we use BottomNav for mobile) */}
      <div className={`hidden md:flex flex-col fixed inset-y-0 left-0 z-50 md:relative w-72 bg-white dark:bg-[#121110] border-r border-gray-100 dark:border-white/10 h-screen overflow-hidden shadow-[4px_0_24px_rgba(0,0,0,0.03)]`}>
        {/* Logo */}
        <div className="h-20 flex items-center justify-between px-8 shrink-0 border-b border-gray-100 dark:border-white/10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-bold text-[#61605D] dark:text-gray-400/50 uppercase tracking-[0.2em]">Owner Portal</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="hidden md:block"><ThemeToggle /></div>
            <button className="md:hidden p-1 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white" onClick={() => setIsMobileMenuOpen(false)}>
              <X className="w-6 h-6" />
            </button>
          </div>
        </div>
