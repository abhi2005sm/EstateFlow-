...
export default function PropertyTypes({ delay = 0 }) {
  return (
    <motion.div
      ...
      className="bg-white/70 backdrop-blur-3xl p-10 rounded-[3rem] border border-white/30 shadow-[0_20px_50px_rgba(0,0,0,0.05)] h-full flex flex-col justify-between"
    >
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-2xl font-black tracking-tight text-[#121110]">Properties Types</h3>
      ...
      <div className="flex items-center flex-1">
        <div className="w-[55%] h-64 relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-4xl font-black text-[#121110]">3,786</span>
        ...
        <div className="w-[45%] grid grid-cols-2 gap-y-10 gap-x-6 pl-10">
          {data.map((item, i) => (
            <div key={i} className="space-y-3">
              ...
              <p className="text-xl font-black text-[#121110] tracking-tight">{item.value.toLocaleString()}</p>
            </div>
          ))}
        </div>
      </div>
