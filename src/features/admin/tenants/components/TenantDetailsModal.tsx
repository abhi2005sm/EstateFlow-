...
import { X, User, Mail, Phone, Home, Users, Utensils, PawPrint, Calendar, DollarSign, ShieldCheck, BadgeCheck, CheckCircle2, IndianRupee, Wrench, Clock } from 'lucide-react';
    ...
    if (isOpen && tenantId) {
      const loadData = async () => {
        ...
        try {
          ...
          const timelineData = await tenantsApi.getTenantTimeline(tenantId);
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-white dark:bg-[#18181b] rounded-[32px] overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto"
        >
          {loading ? (
            <div className="h-[500px] flex flex-col items-center justify-center space-y-4">
              <div className="w-10 h-10 border-4 border-blue-100 border-t-blue-600 rounded-full animate-spin" />
              <p className="text-gray-500 dark:text-gray-400 font-bold text-sm tracking-widest uppercase">Fetching Resident Profile...</p>
            </div>
          ) : tenant ? (
            <>
              {/* Header Banner */}
              <div className="relative h-32 bg-gradient-to-r from-blue-600 to-indigo-700 p-8">
                ...
                <div className="absolute -bottom-12 left-8 p-1 bg-white dark:bg-[#18181b] rounded-[24px] shadow-lg">
                  <div className="w-24 h-24 bg-blue-50 rounded-[20px] flex items-center justify-center">
                    <User className="w-10 h-10 text-blue-600 dark:text-white" />
                  </div>
                </div>
              </div>
              ...
              <div className="pt-16 pb-10 px-8">
                <div className="flex items-start justify-between mb-8">
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <span className="px-2 py-0.5 bg-blue-50 text-blue-600 dark:text-white text-[10px] font-black uppercase tracking-wider rounded-md">
                        {tenant.tenant_type}
                      </span>
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-gray-50 dark:bg-[#27272a] rounded-2xl p-4 grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[
                          ...
                          <div key={i} className="bg-white dark:bg-[#18181b] border border-gray-100 dark:border-gray-800 rounded-xl p-2 text-center">
                ...
                <div className="mt-10 p-6 bg-blue-50/50 rounded-[24px] border border-blue-100/50">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-2">
                      <Calendar className="w-4 h-4 text-blue-600 dark:text-white" />
                      <span className="text-xs font-black text-[#1A1C1E] uppercase tracking-widest">Lease Agreement</span>
                    </div>
                    <span className="text-[10px] font-bold text-blue-600 dark:text-white bg-white dark:bg-[#18181b] px-3 py-1 rounded-full shadow-sm">
                      Next Due: June 1st
                    </span>
                  </div>
                ...
                <div className="mt-10">
                  <div className="flex items-center space-x-3 mb-6">
                    ...
                    <h3 className="text-xl font-black text-[#1A1C1E] dark:text-white tracking-tight">Activity Timeline</h3>
                  </div>
                  
                  <div className="relative pl-6 border-l-2 border-gray-100 dark:border-gray-800 space-y-8 pb-4">
                    {timeline.length === 0 ? (
                      <div className="text-sm font-semibold text-gray-500">No recent activity found.</div>
                    ) : (
                      timeline.map((event: any, idx: number) => {
                        ...
                        return (
                          <motion.div 
                            ...
                            className="relative"
                          >
                            <div className={`absolute -left-[35px] top-1 p-1 rounded-full bg-white dark:bg-[#18181b] border-4 border-white dark:border-[#18181b] shadow-sm`}>
                              <div className={`p-1.5 rounded-full ${iconColor}`}>
                                <Icon className="w-3.5 h-3.5" />
                              </div>
                            </div>
                            <div className="bg-gray-50 dark:bg-[#27272a]/50 border border-gray-100 dark:border-gray-800 rounded-2xl p-5 hover:shadow-md transition-shadow ml-2">
                              <div className="flex flex-col md:flex-row md:items-center justify-between mb-2 space-y-2 md:space-y-0">
                                <h4 className="text-sm font-black text-[#1A1C1E] dark:text-white tracking-tight">{event.title}</h4>
                                <span className="inline-block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest bg-white dark:bg-[#18181b] px-3 py-1.5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800">
                                  {new Date(event.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                </span>
                              </div>
                              <p className="text-xs font-bold text-[#64748B] dark:text-gray-400 leading-relaxed mt-1">
                                {event.description}
                              </p>
                            </div>
                          </motion.div>
            ...
            <div className="p-20 text-center space-y-4">
              ...
              <p className="text-gray-500 dark:text-gray-400 font-bold">Resident details could not be loaded.</p>
